...
class circlePack {
  ...
  scoreRectangle(node) {
    ...
    return Math.max(Math.abs(cx * this.height), Math.abs(cy * this.width));
  }
  ...
  pack(circles) {
    ...
    pack: for (let i = 3; i < n; ++i) {
      this.place(a._, b._, (c = circles[i])), (c = new Node(c));
      ...
      do {
        if (sj <= sk) {
          if (this.intersects(j._, c._)) {
            (b = j), (a.next = b), (b.previous = a), --i;
            continue pack;
          }
          (sj += j._.r), (j = j.next);
        } else {
          if (this.intersects(k._, c._)) {
      ...
      let aa = this.scoreRectangle(a),
        ca;
      while ((c = c.next) !== b) {
        if ((ca = this.scoreRectangle(c)) < aa) {
          (a = c), (aa = ca);
        }
      }
      b = a.next;
